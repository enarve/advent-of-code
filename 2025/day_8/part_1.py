...
def find_closest_points(points, distances):
    min_dist: int = min(min(list(filter(lambda x: x > 0, row))) for row in distances)
    x = (0, 0)
...
def count_circuits(points: list[Point]):
    ...
    for point in points:
        ...
        if c_dict.get(c):
            ...
            c_dict[c] = 1
    c_list = list(reversed(sorted(list(c_dict.items()), key=lambda x: x[1])))
    print(f'1. Circuit {c_list[0][0]} with {c_list[0][1]} connections')
    print(f'2. Circuit {c_list[1][0]} with {c_list[1][1]} connections')
    print(f'3. Circuit {c_list[2][0]} with {c_list[2][1]} connections')
